...
def _item_sort_key(item: KnowledgeRetrievalItem) -> tuple:
    ...
    first; for equal relevance, stronger applicability certainty first
    (MATCH < PARTIAL_MATCH < UNKNOWN); then deterministic identity
    tie-breakers. `version_label` participating here carries ZERO
    governance/precedence meaning -- it is used only because SOME total
    order is required for deterministic output once relevance and
    applicability certainty are already tied; `section.section_id` is
    the final, guaranteed-unique tie-breaker (unique per governed
    object, per 5.1A).
    """
    return (
