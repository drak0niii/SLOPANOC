"""The pragmatic local implementation of `KnowledgeRepository`.
...
class KnowledgeObjectRecord(Base):
    ...
    a Python "check then insert" -- see `SQLiteKnowledgeRepository.add`).
...
class SQLiteKnowledgeRepository:
    """The local, pragmatic `KnowledgeRepository` implementation.
    ...
    callers supply a real file path, a temporary path, or an in-memory
    URL (e.g. `"sqlite+aiosqlite:///:memory:"`). Runtime configuration
    wiring (a process-wide singleton bound to
    `Settings.resolve_database_url()`, mirroring
    `backend/cases/db.py`'s `get_case_database()`) is explicitly out of
